# Defect_detection with 3p cropping
...
def crop_vertical_region(image, top_crop_pct, bottom_crop_pct):
    h, w = image.shape[:2]

    top = int(h * top_crop_pct)
    bottom = int(h * (1 - bottom_crop_pct))
...
def compare_images(reference_image_path, input_image_path, min_contour_area=100, threshold_value=30):
    ...
    reference_image = crop_vertical_region(reference_image, 0.20, 0.10)
    cv2.imwrite("reference_image_cropped.png", reference_image)
    input_image = crop_vertical_region(input_image, 0.20, 0.10)
    cv2.imwrite("input_image_cropped.png", input_image)
    ...
    reference_blurred = cv2.GaussianBlur(reference_gray, (67,67), 0)
    input_blurred = cv2.GaussianBlur(input_gray, (67,67), 0)
...
if __name__ == "__main__":
    ...
    input_image=r"C:\ALL PROJECTS\MCCB-Defect-Detection\Test_images\P1_4P_Testing.png"
    ...
    # detect defects
    ref_image_file = r"C:\ALL PROJECTS\MCCB-Defect-Detection\masterImages\cropped_masterP14P_mccb.png"
